...
PreCustomerMSA/
  ...
  precustomer-core/          # shared library repo
...
- `precustomer-contracts`는 Gateway와 core가 공유하는 Passport 계약을 담는 작은 라이브러리 repo다.
- `precustomer-core`는 공통 도메인, repository, 서비스, 템플릿, 설정 리소스를 담는 라이브러리 repo다.
- public/admin/worker repo는 `com.example:precustomer-core:0.0.1-SNAPSHOT`에 의존한다.
...
- 운영/CI에서는 core를 Maven local, GitHub Packages, Nexus 같은 registry에 publish한 뒤 service repo가 version dependency로 받으면 된다.
...
```bash
cd ../PreCustomerMSA/precustomer-core
./gradlew test
...
1. GitHub에 `precustomer-contracts`, `precustomer-core`, `precustomer-api-gateway`, `precustomer-public-web`, `precustomer-admin-web`, `precustomer-report-worker`, `precustomer-platform` repo 생성
...
4. core 라이브러리를 GitHub Packages나 사설 Maven registry로 publish
5. service repo의 composite build fallback은 유지하되 CI에서는 registry dependency로 빌드
