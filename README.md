...
### public/admin/worker 분리 실행

로컬에서 사용자 화면, 관리자 화면, 리포트 worker를 별도 프로세스로 분리해 확인할 수 있습니다.
...
# 터미널 3: report_job 큐를 처리하는 worker
./gradlew bootRunWorker
```

운영에서는 같은 JAR를 세 프로세스로 실행합니다.

```bash
java -jar build/libs/precustomer-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod,public-web
java -jar build/libs/precustomer-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod,admin-web
java -jar build/libs/precustomer-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod,worker
```

자세한 분리 운영 방식은 [`docs/report-worker-migration.md`](docs/report-worker-migration.md)를 참고하세요.
