...
브라우저에서 `http://localhost:8080` 접속. 이 주소는 사용자 화면 전용입니다.

사용자 회원가입/로그인은 public-web에서 처리합니다.

| 화면 | 직접 실행 URL | Gateway 경유 URL |
| --- | --- | --- |
| 사용자 홈 | `http://localhost:8080/` | `http://localhost:8088/` |
| 사용자 로그인 | `http://localhost:8080/login` | `http://localhost:8088/login` |
| 사용자 회원가입 | `http://localhost:8080/signup` | `http://localhost:8088/signup` |
| 내 리포트 | `http://localhost:8080/account/reports` | `http://localhost:8088/account/reports` |
| 관리자 로그인 | `http://localhost:8081/admin/login` | `http://localhost:8088/auth/login` |
| 관리자 주문 목록 | `http://localhost:8081/admin/orders` | `http://localhost:8088/admin/orders` |

MSA 분리 후 `localhost:8080`에는 관리자 화면이 뜨지 않습니다. 관리자 화면은 `admin-web` 직접 접속 시 `8081`, Gateway 경유 시 `8088`을 사용합니다. Gateway 경유 관리자 접속은 먼저 `/auth/login`에서 로그인한 뒤 `/admin/orders`로 이동합니다.
...
값을 지정하지 않으면 개발용 fallback 계정이 적용될 수 있습니다. 로컬 개인 테스트를 제외한 모든 환경에서는 반드시 환경변수로 바꿔서 실행합니다.
...
export PUBLIC_SQL_INIT_MODE=always              # public-web local PostgreSQL 사용자 인증 테이블 자동 보정
```
...
`public-web`의 local 프로필은 `schema-postgresql.sql`을 실행해 `user_account`, `report_delivery_request`, `analytics_event_log` 누락을 보정합니다. 전체 DB를 처음 구성하는 경우에는 아래 DB 마이그레이션 SQL을 먼저 실행하세요.
...
| `GET /login` | 사용자 로그인 |
| `GET /signup` | 사용자 회원가입 |
| `GET /account/reports` | 로그인한 사용자의 최종 리포트 목록 |
| `GET /account/reports/{id}` | 로그인한 사용자의 최종 리포트 상세 |
| `GET /orders/new` | 리포트 신청 폼 |
| `POST /orders` | 신청 저장 (이미지 업로드 포함) |
| `GET /admin/login` | 관리자 로그인 |
| `GET /admin/orders` | 관리자 주문 목록 |
| `GET /admin/orders/{id}` | 관리자 주문 상세 + 리포트 생성 |
...
psql -h localhost -U postgres -d precustomer -f docs/sql/11_user_auth_report_delivery.sql
psql -h localhost -U postgres -d precustomer -f docs/sql/12_msa_physical_database_split.sql
psql -h localhost -U postgres -d precustomer -f docs/sql/13_analytics_event_log.sql
psql -h localhost -U postgres -d precustomer -f docs/sql/14_user_account_terms_consent.sql
```

이번 `user_account` 오류는 `11_user_auth_report_delivery.sql`이 적용되지 않은 DB에서 사용자 로그인/회원가입이 실행될 때 발생합니다. local public-web은 누락 테이블을 자동 보정하지만, 운영 DB는 위 SQL을 명시적으로 적용하는 것을 권장합니다.
