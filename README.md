...
로컬에서 web과 리포트 worker를 분리해 확인할 수 있습니다.

```bash
# 터미널 1: HTTP 요청/관리자 화면만 담당
./gradlew bootRunWeb

# 터미널 2: report_job 큐를 처리하는 worker만 담당
./gradlew bootRunWorker
```

운영에서는 같은 JAR를 두 프로세스로 실행합니다.

```bash
java -jar build/libs/precustomer-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod,web
...
export REPORT_PIPELINE_WORKER_ENABLED=false     # web 프로세스에서는 false
```
